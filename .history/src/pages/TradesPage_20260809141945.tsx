...
const TradesPage: React.FC = () => {
  ...
  const handleSearch = () => {
    // Real-time filtering is already in place; this is a visual trigger.
    // Could be extended to call backend API for server-side search.
    console.log('Searching with filters:', { searchSymbol, searchEntryDateStart, searchEntryDateEnd, searchStatus });
  };
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6 bg-white dark:bg-gray-800 p-4 px-0 rounded-lg shadow-sm">
        ...
        <div className="flex items-center gap-1 ml-4">
          <Button variant="gradient" onClick={() => handleQuickFilter('today')} className={`text-xs py-1 px-3 ${quickFilter === 'today' ? 'bg-purple-600 text-white' : ''}`}>Today</Button>
          <Button variant="gradient" onClick={() => handleQuickFilter('week')} className={`text-xs py-1 px-3 ${quickFilter === 'week' ? 'bg-purple-600 text-white' : ''}`}>This Week</Button>
          <Button variant="gradient" onClick={() => handleQuickFilter('month')} className={`text-xs py-1 px-3 ${quickFilter === 'month' ? 'bg-purple-600 text-white' : ''}`}>This Month</Button>
          <Button variant="gradient" onClick={() => handleQuickFilter('year')} className={`text-xs py-1 px-3 ${quickFilter === 'year' ? 'bg-purple-600 text-white' : ''}`}>This Year</Button>
        </div>
        ...
        <div className="flex items-center gap-1">
          <input
            type="date"
            value={searchEntryDateStart}
            onChange={(e) => setSearchEntryDateStart(e.target.value)}
            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
          />
          <span className="text-gray-400">~</span>
          <input
            type="date"
            value={searchEntryDateEnd}
            onChange={(e) => setSearchEntryDateEnd(e.target.value)}
            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
          />
        </div>
        ...
        <Button variant="primary" onClick={handleSearch} className="text-sm py-2">Search</Button>
        ...
        <Button variant="gradient" onClick={() => navigate('/stats')} className="text-sm py-2">Statistics</Button>
      </div>
