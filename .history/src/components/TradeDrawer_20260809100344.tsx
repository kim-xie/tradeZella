...
import { createTrade, updateTrade, CreateTradeData } from '../services/api';
    ...
    useEffect(() => {
        if (trade) {
            setFormData({
                ...
                notes: trade.notes || '',
                tradeDate: trade.createdat ? new Date(trade.createdat).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
            });
        } else {
            setFormData({
                symbol: '',
